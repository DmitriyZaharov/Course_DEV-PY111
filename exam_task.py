...
"""
   4. Навигатор на сетке.
    ...
    Минимальную стоимость пути будем находить с помощью заполнения матрицы накопленных стоимостей,
    а сам путь восстанавливать с помощью обратного хода от конечной ячейки к начальной.
    
    Будем считать, что перемещаться можно в четырех направлениях (вверх, вниз, влево, вправо). Так как сетка 
    плоская и квадратная. 
    
    1. Создадим матрицу dp такого же размера, как и исходная сетка, заполненную бесконечностью.
    В начальную ячейку запишем её собственную стоимость. Также создадим матрицу parent для хранения
    координат предыдущей ячейки, чтобы в конце восстановить путь.
    
    2. Используем кучу (heap), чтобы всегда первыми обрабатывать ячейки с наименьшей текущей стоимостью.
    
    3. Для текущей ячейки проверяем 4 соседних направления (вверх, вниз, влево, вправо).
    
    4. Если путь в соседнюю ячейку через текущую выгоднее, чем найденный ранее, обновляем стоимость и запоминаем предка.
    
    5. Идем от конечной точки к стартовой по сохраненным предкам, затем разворачиваем список.
    
"""
def find_min_cost_path(grid, start, end):
    """
    Находит путь минимальной стоимости в сетке.

    Args:
        grid: двумерный список положительных чисел (стоимость захода в ячейку)
        start: координаты старта (rows, cols)
        end: координаты финиша (rows, cols)

    Returns:
        (минимальная стоимость, список координат пути от start до end)
        Если путь не найден, возвращает (inf, None)
    """
    rows, cols = len(grid), len(grid[0])

    # Матрица минимальных стоимостей до каждой ячейки
    distances = [[float('inf')] * cols for _ in range(rows)]

    # Начальная стоимость — это стоимость захода в стартовую ячейку
    distances[start[0]][start[1]] = grid[start[0]][start[1]]

    # Словарь для восстановления пути: (r, c) -> (parent_r, parent_c)
    parent = {}

    # Очередь с приоритетом: (текущая_стоимость, строка, столбец)
    queue = [(distances[start[0]][start[1]], start[0], start[1])]
    ...
    while queue:
        ...
        # Если нашли путь короче, пропускаем устаревшую запись
        ...
        for dr, dc in directions:
            ...
            if 0 <= nr < rows and 0 <= nc < cols:
                # Стоимость захода в соседнюю ячейку
                new_dist = current_dist + grid[nr][nc]

                # Если новый путь дешевле, обновляем данные
                if new_dist < distances[nr][nc]:
                    distances[nr][nc] = new_dist
                    parent[(nr, nc)] = (r, c)
                    heapq.heappush(queue, (new_dist, nr, nc))

    # Если конечная точка недостижима (путь не найден)
    ...
    while curr != start:
        ...
        curr = parent[curr]
    path.append(start)
    # Разворачиваем, чтобы идти от старта к финишу
    path.reverse()
...
if __name__ == "__main__":
    ...
    best_path, min_cost = find_min_cost_path(grid, start, end)
    ...
    best_path, min_cost = find_min_cost_path(matrix, start_point, end_point)
    print(f"Путь: {best_path}")
    print(f"Минимальная стоимость пути: {min_cost}")
    print("-" * 40)
